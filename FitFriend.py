...
class FitFrenApp(tk.Frame): #Creates overall window for FitFriend
    ...
    def create_button_area(self):
        ...
        self.button_frame = tk.Frame(self.main, bg='blue', height= 50, width=150)
...
class MainPage(tk.Frame): #Landing page for FitnessFriend
    ...
    def create_display_area(self):
        ...
        self.display_label = tk.Label(self.display_frame, fg = self.text_color, bg = self.bg_color, font = ('Arial', 14, 'bold'), 
                                      text = 'Welcome! Please select a page from below. FitnessFriend is here to help YOU reach all your fitness goals!', 
                                      height = 10, width = 50, wraplength = 500, anchor = 'w')
...
class Tracker(tk.Frame): #Log for calorie intake for each day of the week
    ...
    def daily_tracker(self): #Display calorie count per day of the week based on selected meals
        ...
        self.tracker_frame = tk.Frame(self.display_frame, height= 500, width=800, bg='purple')
        ...
        for i, day in enumerate(days):
            ...
            self.day_label = tk.Label(self.tracker_frame, fg = self.text_color, bg = self.bg_color, text = day,  height = 2, width = 20, wraplength = 200)
            ...
            self.cals_entry.grid(row=i+2, column=1, ipadx=5, ipady=1, pady=2)
            
            self.enter_button = tk.Button(self.tracker_frame, fg = self.text_color, bg='black', text='Submit', command=lambda: self.get_cal_list(self.cals.get()))
            self.enter_button.grid(row=i+2, column=2, ipadx=1, ipady=1, pady=2)
    
       
        #Enter average calories to keep track of for the week
        self.avg_label = tk.Label(self.tracker_frame, fg = self.text_color, bg = self.bg_color, text = "Average Weekly Calories:", height = 2, width = 20, wraplength = 200)
        ...
        self.avgcals_entry.grid(row=9, column=1, ipadx=5, ipady=1, pady=2)
            
        self.entry_button = tk.Button(self.tracker_frame, fg = self.text_color, bg='black', text='Submit', command=lambda: self.get_cal_list(self.cals.get()))
        self.entry_button.grid(row=9, column=2, ipadx=1, ipady=1, pady=2)
        
        #save button to return for another session
        save_button = tk.Button(self.tracker_frame, command = lambda: self.save_data(self.cals, str(self.cals)))
        ...
        self.avg_label = tk.Label(self.tracker_frame, fg = self.text_color, bg = self.bg_color, text = "Sample Meals:", height = 2, width = 20, wraplength = 200).grid(row = 1, column=10, sticky = 'e')
        self.avg_label = tk.Label(self.tracker_frame, fg = self.text_color, bg = self.bg_color, text = "Calories:", height = 2, width = 20, wraplength = 200).grid(row = 1, column=11, sticky ='e')
        
        row = 2
        for meal, calories in self.brekkie_dict.items():
            tk.Label(self.tracker_frame, text=meal, width=30, borderwidth=2, relief="solid").grid(row=row, column=10, ipadx=5, ipady=5)
            tk.Label(self.tracker_frame, text=calories, width=15, borderwidth=2, relief="solid").grid(row=row, column=11, ipadx=5, ipady=5)
            row += 1
